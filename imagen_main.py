...
def conditioning_pred(generator_state, t, cond_scale):
    pred = generator_state.imagen_state.train_state.apply_fn(
        {"params": generator_state.imagen_state.train_state.params}, generator_state.image, t, generator_state.text, generator_state.attention, 0.0, generator_state.rng)
    null_logits = generator_state.imagen_state.train_state.apply_fn(
        {"params": generator_state.imagen_state.train_state.params}, generator_state.image, t, generator_state.text, generator_state.attention, 1.0, generator_state.rng)
...
def p_mean_variance(t_index, generator_state):
    ...
    x_start = generator_state.imagen_state.sampler.predict_start_from_noise(
...
def p_sample_loop(imagen_state, img, texts, attention, rng):
    ...
    generator_state = GeneratorState(
        imagen_state=imagen_state, image=img, text=texts, attention=attention, rng=rng)
...
def train_step(imagen_state, imgs_start, timestep, texts, attention_masks, rng):
    ...
    def loss_fn(params):
        predicted_noise = imagen_state.train_state.apply_fn(
            {"params": params}, x_noise, timestep, texts, attention_masks, imagen_state.conditional_drop_prob, key2)
