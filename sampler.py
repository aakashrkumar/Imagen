...
class GaussianDiffusionContinuousTimes(nn.Module):
    ...
    def setup(self):
        ...
        self.alphas_cumprod_prev = jnp.pad(self.alphas_cumprod, ((1, 0),), constant_values=1.0)
        ...
        self.posteiror_variance = self.betas * (1. - self.alphas_cumprod_prev) / (1. - self.alphas_cumprod)
...
def test():
    ...
    scheduler = GaussianDiffusionContinuousTimes(noise_schedule="cosine", num_timesteps=1000)
    images = []
    for i in range(1000):
    ...
    videoWriter = cv2.VideoWriter("video.mp4", cv2.VideoWriter_fourcc(*'MJPG'), 240, (x_noise.shape[1], x_noise.shape[0]))
    for i in range(1000):
