...
class Attention(nn.Module):
    ...
    def __call__(self, x, context=None, mask=None, attn_bias=None):
        ...
        q = nnp.Dense(features=inner_dim, use_bias=False,shard_axes={"kernel": ("embed_kernel", "mlp")})(x)
        k, v = nnp.Dense(features=self.dim_head * 2, use_bias=False,shard_axes={"kernel": ("embed_kernel", "mlp")})(x).split(2, axis=-1)
        ...
        if exists(context):
            ...
            context_hidden = nnp.Dense(features=self.dim_head*2,shard_axes={"kernel": ("embed_kernel", "mlp")})(context_hidden)
        ...
        out = nnp.Dense(features=self.dim, use_bias=False,shard_axes={"kernel": ("embed_kernel", "mlp")})(out)
...
class CrossAttention(nn.Module):
    ...
    def __call__(self, x, context, mask=None):
        ...
        q = nnp.Dense(features=inner_dim, use_bias=False, shard_axes={"kernel": ("embed_kernel", "mlp")})(x)
        k, v = nnp.Dense(features=inner_dim * 2, use_bias=False, shard_axes={"kernel": ("embed_kernel", "mlp")})(context).split(2, axis=-1)
...
class TextConditioning(nn.Module):
    ...
    def __call__(self, text_embeds, text_mask, time_cond, time_tokens, rng):
        if exists(text_embeds):
            text_tokens = nnp.Dense(features=self.cond_dim, shard_axes={"kernel": ("embed_kernel", "mlp")})(text_embeds)
            ...
            text_hiddens = nnp.Dense(features=self.time_cond_dim, shard_axes={"kernel": ("embed_kernel", "mlp")}, dtype=self.dtype)(text_hiddens)
...
class ResnetBlock(nn.Module):
    ...
    def __call__(self, x, time_emb=None, cond=None):
        ...
        if exists(time_emb):
            ...
            time_emb = nnp.Dense(features=self.dim * 2, dtype=self.dtype, shard_axes={"kernel": ("embed_kernel", "mlp")})(time_emb)
